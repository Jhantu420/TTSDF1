...
const registerAdmin = async (req, res) => {
  try {
    const { adminId, name, email, password, role, branchName } = req.body;

    if (!email || !name || !password || !role || !branchName) {
    ...
    if (!adminId) {
      ...
      newAdminId = `${role}/${branchName}/${newNumber}`;
    }
...
const createBranchAdmin = async (req, res) => {
  try {
    ...
    if (!admin) {
      ...
      adminId = `${role}-${branchName}-${newNumber}`;
    } else {
...
const updateBranchAdmin = async (req, res) => {
  try {
    ...
    if (requestingAdmin.role !== "super" || !requestingAdmin.activeStatus) {
      return res.status(403).json({
        message: "Access denied. Only active Super Admins can update branch admins.",
      });
    }
    ...
    if (branchName && branchName !== existingAdmin.branchName || (role && role !== existingAdmin.role)) {
      const lastAdmin = await adminModel
        .findOne({ role: role || existingAdmin.role, branchName: branchName || existingAdmin.branchName })
        .sort({ createdAt: -1 });
      ...
      updatedAdminId = `${role || existingAdmin.role}-${branchName || existingAdmin.branchName}-${newNumber}`;
    }
    ...
    const updatedAdmin = await adminModel.findByIdAndUpdate(
      id,
      {
        ...
        activeStatus: activeStatus !== undefined ? activeStatus : existingAdmin.activeStatus,
